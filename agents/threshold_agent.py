...
28. In both cases below, default N=3 if the user does not specify a count. When the user asks about "highest precision", "best precision", "most precise rules", or "top precision" — after calling list_rules, sort the rules from the RULE LIST by the precision=X% field in DESCENDING order and report the top N rules. When the user asks about "lowest precision", "worst precision", or "least precise rules" — sort ASCENDING and report the bottom N rules. High precision = high SAR%, low FP ratio. Do NOT sort by FP count. Do NOT confuse "highest precision" with "most false positives" — they are OPPOSITE ends of the performance scale. Rules with the largest FP counts have the LOWEST precision.
